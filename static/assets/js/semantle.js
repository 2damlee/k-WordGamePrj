...
let numPuzzles = 4650;
const now = Date.now();
const initialDate = new Date('2023-03-01T00:00:00+09:00');
...
function guessRow(similarity, oldGuess, percentile, guessNumber, guess) {
    ...
    let progress = "";
    let closeClass = "";
    if (similarity >= similarityStory.rest * 100 && percentile === 'OUT OF 1000 :(') {
        percentileText = '<span class="weirdWord">????<span class="tooltiptext">이 단어는 사전에는 없지만, 데이터셋에 포함되어 있으며 1,000순위 이내입니다.</span></span>';
    }
    if (typeof percentile === 'number') {
closeClass = "close";
percentileText = ``
progress = ` <span class="progress-container">
<span class="progress-bar" style="width:${(1001 - percentile)/10}%">&nbsp;<div class="percentage-text">${percentile}/1000</div></span>
</span>`;
    }
    let style = '';
    if (oldGuess === guess) {
...
function solveStory(guesses, puzzleNumber) {
    ...
    if (is_win) {
        guess_count += 1
        if (guess_count == 1) {
            return `첫번째 추측에서 ${puzzleNumber}번째 정답 단어를 맞혔습니다!\nhttps://semantle-ko.newsjel.ly/`;
        }
    }
    if (guess_count == 0) {
        return `${puzzleNumber}번째 수수께끼단어를 시도하지 않고 포기했어요.\nhttps://semantle-ko.newsjel.ly/`;
    }

    let describe = function(similarity, percentile) {
        let out = `${similarity.toFixed(2)}`;
        if (percentile != 'OUT OF 1000 :(') {
            out += ` (순위 ${percentile})`;
        }
        return out;
    }
    ...
    if (is_win) {
        return `${puzzleNumber}번째 단어를 풀었습니다!\n${guessCountInfo}` +
            `${timeInfo}${topGuessMsg}https://semantle-ko.newsjel.ly/`;
    }

    return `${puzzleNumber}번째 단어를 포기했어요\n${guessCountInfo}` +
            `${timeInfo}${topGuessMsg}https://semantle-ko.newsjel.ly/`;
}
...
let Semantle = (function() {
    ...
    async function init() {
        ...
        try {
            ...
            오늘의 수수께끼 단어는 ${puzzleNumber}번째 입니다. 
            정답 단어와 가장 비슷한 단어의 유사도는 <b>${(similarityStory.top * 100).toFixed(2)}</b> 입니다.
    ...
    function updateGuesses(guess) {
        let inner = `<tr><th id="chronoOrder">#</th><th id="alphaOrder">추측 단어</th><th id="similarityOrder">유사도</th><th>얼마나 비슷한가요?</th></tr>`;
...
<b>나의 플레이 기록</b>: <br/>
<table>
<tr><th>가장 처음 풀었던 단어 번호:</th><td>${stats['firstPlay']}</td></tr>
<tr><th>게임 횟수:</th><td>${totalGames}</td></tr>
